...
namespace QA.DotNetCore.Engine.Routing.UrlResolve
{
    ...
    public class Url
    {
        ...
        public Url AppendQuery(string key, string value, bool unlessNull)
        {
            ...
            return AppendQuery(key + "=" + WebUtility.UrlEncode(value));
        }
        ...
        public Url SetQueryParameter(string key, string value, bool removeNullValue)
        {
            ...
            for (int i = 0; i < queries.Length; i++)
            {
                if (queries[i].StartsWith(key + "=", StringComparison.OrdinalIgnoreCase))
                {
                    if (value != null)
                    {
                        queries[i] = key + "=" + WebUtility.UrlEncode(value);
        ...
        {
            return new Url(_scheme, authority, _path, _query, _fragment, _forcedAddTrailingSlashes);
        }
